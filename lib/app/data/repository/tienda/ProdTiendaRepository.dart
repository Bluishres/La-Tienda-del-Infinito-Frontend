...
class ProdTiendaRepository implements TiendaRepository {
  ...
  @override
  Future<Ticket> Comprar({String fecha, double importe, int unidades, int id_usuario, int id_producto}) async{
    await Future.delayed(Duration(seconds: 2));
    try {
      var data = await apiService.requestPostwithParams(Endpoint.comprar,false, fecha: fecha, importe: importe, unidades: unidades, id_usuario: id_usuario, id_producto: id_producto);
  ...
  @override
  Future<ListaUsuariosDeseado> addFavorito({int id_usuario, int id_producto}) async{
    await Future.delayed(Duration(seconds: 2));
    try {
      var data = await apiService.requestPostwithParams(Endpoint.addFavorito,true, id_usuario: id_usuario, id_producto: id_producto);
  ...
  @override
  Future<List<ListaUsuariosDeseado>> getAllfavoritossByUser({int id_user}) async{
    await Future.delayed(Duration(seconds: 2));
    try {
      var data = await apiService.requestGetID(Endpoint.addFavorito, id: id_user);
  ...
  Future<List<Ticket>> getAllticketsByUser({int id_user}) async {
    await Future.delayed(Duration(seconds: 2));
    try {
      var data = await apiService.requestGetID(Endpoint.comprar, id: id_user);
